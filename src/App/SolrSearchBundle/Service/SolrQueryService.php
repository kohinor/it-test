...
class SolrQueryService
{
    ...
    public function setFacets(Query $query, $facets)
    {
        ...
        $facetSet->createFacetField('category')->setField('category')->setSort('index');
        $facetSet->createFacetField('subcategory')->setField('subcategory')->setSort('index');
        $facetSet->createFacetPivot('categories')->addFields('category1,category2');

        if (!empty($facets)) {
            $groupedFacets = array();
            foreach($facets as $facet){
                $groupedFacets[$facet->field][] = $facet->field.':"'.trim($facet->facet).'"';
            }
