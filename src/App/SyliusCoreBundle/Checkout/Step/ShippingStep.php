...
use Sylius\Bundle\CoreBundle\Checkout\Step\ShippingStep as Base;
 ...
 */
class ShippingStep extends Base
{
    /**
    ...
    protected function renderStep(ProcessContextInterface $context, OrderInterface $order, FormInterface $form)
    {
        ...
        foreach($order->getItems() as $item) {
            if ($item->getVariant()->getShippingCategory()) {
                $groupedItems[$item->getVariant()->getProduct()->getShippingCategory()->getId()][] = $item->getVariant();
            } else {
                $groupedItems[0][] = $item->getVariant();
            }
        }
