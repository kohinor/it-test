...
class UploadProductCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $sql = "UPDATE sylius_product_variant as v set v.deleted_at = null where v.product_id in (SELECT id from sylius_product p where p.deleted_at is null);";
        $this->getEM()->getConnection()->exec($sql);
    ...
    protected function updateProduct(\App\SyliusProductBundle\Entity\ProductDropship $productDropship, $output)
    {
        ...
        if (!$product) {
            ...
            if ($productDropship->getModels()->first() && $productDropship->getModels()->first()->getColor()) {
